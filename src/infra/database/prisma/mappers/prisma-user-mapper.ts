...
import type { User, CreateUserData, UpdateUserData } from '@/domain/main/enterprise/entities/user.js'
...
export class PrismaUserMapper {
  private static readonly roleMap = new Map<string, UserRole>(
    Object.entries(USER_ROLES).map(([key, value]) => [value, value as UserRole]),
  )

  static toDomain(raw: UserModel): User {
    const role = this.mapPrismaRoleToDomain(raw.role as string)
    return {
      id: raw.id,
